import numpy as np
import pandas as pd
from pulp import LpMinimize, LpProblem, LpVariable, lpSum, LpBinary, LpContinuous, value, LpStatus
...
model = LpProblem("MedicalWasteCollection", LpMinimize)

# Decision variables
x = LpVariable.dicts("x", (range(number_nodes), range(number_nodes), range(number_vehicles), range(final_time)),
                     0, 1, LpBinary)
W = LpVariable.dicts("W", (range(number_demand_nodes), range(final_time)), 0, None, LpContinuous)
l = LpVariable.dicts("l", (range(number_vehicles), range(final_time)), 0, None, LpContinuous)
WX = LpVariable.dicts("WX", (range(number_demand_nodes), range(number_vehicles), range(final_time)),
                      0, None, LpContinuous)

# Objective functions
f1 = lpSum(traveling_costs[v] * lpSum(
    traveling_times[i, j, v] * x[i][j][v][t] for i in range(number_nodes) for j in range(number_nodes)
    for t in range(final_time))
           for v in range(number_vehicles))
f2 = lpSum(beta[i] * lpSum(W[i][t] for t in range(final_time)) for i in range(number_demand_nodes))
...
M = (number_nodes + 1) * (number_nodes + 1)  # Big M value for constraints

for t in range(final_time):
    for v in range(number_vehicles):
        # Constraint 2: No vehicle can traverse more than one edge simultaneously
        model += lpSum(x[i][j][v][t] for i in range(number_nodes) for j in range(number_nodes)) <= 1, f'cons2_{v}_{t}'
        # Constraint 14: Maximum vehicle load at each time step
        model += l[v][t] <= vehicles_capacities[v], f'cons14_{v}_{t}'
        if t < final_time - 1:
            # Constraint 13: Load adjustment at each vehicle over time
            model += (l[v][t] -
                      M * lpSum(x[p_c][i][v][t + 1]
                                for p_c in processing_indices
                                for i in range(number_nodes)) +
                      lpSum(WX[p_d][v][t + 1] for p_d in demand_indices) <= l[v][t + 1], f'cons13{v}_{t}')
        if t > 0:
            for i in demand_indices:
                # Constraint 15: Waste collection at each demand node
                model += (WX[i][v][t] >= W[i][t - 1] + alpha[i] - M * (
                        1 - lpSum(x[i][j][v][t] for j in range(number_nodes))), f'cons15_{i}_{v}_{t}')

for v in range(number_vehicles):
    # Constraint 1: Each vehicle must start its route at its starting parking node
    model += lpSum(x[starting_nodes[v]][j][v][0] for j in demand_indices) == 1, f'cons1_{v}'
    # Constraint 8: Each vehicle must visit one parking node immediately after visiting a processing node
    model += lpSum(x[p_c][p_p][v][t] for p_c in processing_indices for p_p in parking_indices
                   for t in range(final_time)) == 1, f'cons8_{v}'
    # Constraint 12: Initial load of each vehicle
    model += l[v][0] == 0, f'cons12_{v}'
    for i in range(number_nodes):
        # Constraint 6: No vehicle can remain stationary at any node
        model += lpSum(x[i][i][v][t] for t in range(final_time)) == 0, f'cons6_{i}_{v}'
        for t in range(final_time):
            if t > 0 and i not in parking_indices:
                ...
                model += (lpSum(x[i][k][v][t] for k in range(number_nodes) if k != i) <=
                          lpSum(x[h][i][v][t - traveling_times[h, i, v]] for h in range(number_nodes)
        ...
        for j in range(number_nodes):
            if i != j:
                ...
                for t in range(final_time - travel_time):
                    model += (x[i][j][v][t] + lpSum(x[j][k][v][t_sum] for k in range(number_nodes)
                    ...
                    if j not in parking_indices:
                        ...
                        model += (x[i][j][v][t] <= lpSum(x[j][k][v][t + travel_time]
...
for j in demand_indices:
    ...
    model += lpSum(x[i][j][v][t] for i in range(number_nodes) for v in range(number_vehicles)
                   for t in range(final_time)) >= 1, f'cons7_{j}'
    # Constraint 10: Initial amount of waste at each demand node
    model += W[j][0] == 0, f'cons10_{j}'
    for t in range(final_time - 1):
        # Constraint 11: Amount of waste at each demand node over time
        model += (W[j][t + 1] >= W[j][t] + alpha[j] - M * lpSum(x[j][i][v][t + 1]
                                                                for i in range(number_nodes)
                                                                for v in range(number_vehicles)), f'cons11_{j}_{t}')
...
for p_p in parking_indices:
    for v in range(number_vehicles):
        for t_prime in range(final_time - 1):
            model += (M * (1 - lpSum(x[i][p_p][v][t_prime] for i in range(number_nodes))) >=
                      lpSum(x[i][j][v][t] for i in range(number_nodes) for j in range(number_nodes)
...
if LpStatus[model.status] == 'Optimal':
    ...
    for v in range(number_vehicles):
        ...
        for t in range(final_time):
            for j in range(number_nodes):
                if value(x[current_node][j][v][t]) > 0.5:
